...
def create_top_contact_mesh(mesh_name, device_name, L_c, L_gap, thickness, mesh_factor=1.0):
    """
...
def create_edge_contact_mesh(mesh_name, device_name, L_gap, thickness, mesh_factor=1.0):
    ...
    # Y coordinates (vertical) - Top surface is at y=0, bottom is at y=thickness
    add_2d_mesh_line(mesh=mesh_name, dir="y", pos=0.0, ps=(thickness/5)/mesh_factor)
    add_2d_mesh_line(mesh=mesh_name, dir="y", pos=thickness, ps=(thickness/5)/mesh_factor)
    add_2d_mesh_line(mesh=mesh_name, dir="y", pos=2*thickness, ps=(thickness/5)/mesh_factor)
    
    # Regions
    add_2d_region(mesh=mesh_name, material="InSe", region="bulk", xl=0.0, xh=L_gap, yl=0.0, yh=thickness)
    add_2d_region(mesh=mesh_name, material="metal", region="source_metal", xl=-L_dummy, xh=0.0, yl=0.0, yh=thickness)
    add_2d_region(mesh=mesh_name, material="metal", region="drain_metal", xl=L_gap, xh=L_gap+L_dummy, yl=0.0, yh=thickness)
    add_2d_region(mesh=mesh_name, material="metal", region="substrate_metal", xl=-L_dummy, xh=L_gap+L_dummy, yl=thickness, yh=2*thickness)
    
    # Add Contacts on the vertical edges (x=0 and x=L_gap)
    add_2d_contact(mesh=mesh_name, name="source", material="metal", region="bulk", xl=0.0, xh=0.0, yl=0.0, yh=thickness, bloat=1e-10)
    add_2d_contact(mesh=mesh_name, name="drain", material="metal", region="bulk", xl=L_gap, xh=L_gap, yl=0.0, yh=thickness, bloat=1e-10)
    
    # Add Thermal Contact on the bottom surface (y=thickness)
    add_2d_contact(mesh=mesh_name, name="substrate", material="metal", region="bulk", xl=0.0, xh=L_gap, yl=thickness, yh=thickness, bloat=1e-10)

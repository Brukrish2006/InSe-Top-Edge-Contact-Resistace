...
def main():
    ...
    for i, Gamma in enumerate(gammas):
        plt.semilogy(wfs, results_top[Gamma], '^--', color=colors[i], label=rf'Top, $\Gamma$ = {Gamma}')
        plt.semilogy(wfs, results_edge[Gamma], 'o-', color=colors[i], label=rf'Edge, $\Gamma$ = {Gamma}')
        
    plt.xlabel(r'Metal Work Function $\Phi_M$ (eV)')
    plt.ylabel(r'Total Resistance ($\Omega\cdot$cm)')
    plt.title(r'Schottky-to-Ohmic Transition Sensitivity to Tunneling Enhancement ($\Gamma$)')
    plt.grid(True, which="both", ls="--", alpha=0.5)
    plt.legend()
    plt.tight_layout()
    plt.savefig('fig_gamma_sensitivity.png', dpi=300)
    print("Gamma sensitivity study completed.")
